...
test.describe('Extension UI modals', () => {
  ...
  test('exposes project trust controls', async ({ page }) => {
    ...
    await expect(settingsDialog.getByText('Project trust')).toBeVisible();
    await settingsDialog.getByRole('button', { name: 'Trust project' }).click();
    expect(
      clientMessages.some((msg) => msg.type === 'set_project_trust' && msg.decision === 'trusted')
    ).toBe(true);
  });
});
