<script lang="ts">
  ...
  interface Props {
    ...
    onSelectOptionFocus?: (index: number) => void;
...
>
  <Dialog.Content
    class={modal?.method === 'custom' && modal.interactive
      ? 'fixed inset-0 top-0 left-0 z-[60] flex h-full w-full max-w-none translate-x-0 translate-y-0 items-center justify-center rounded-none bg-base-100/76 p-0 px-3 py-6 shadow-none ring-0 backdrop-blur-md sm:px-6'
      : 'flex w-full max-w-[calc(100%-1.5rem)] min-h-0 max-h-[min(42rem,calc(100dvh-2rem))] flex-col gap-0 overflow-hidden rounded-2xl border border-base-content/10 bg-base-100/95 p-0 shadow-2xl shadow-black/40 ring-1 ring-base-content/5 backdrop-blur-xl sm:max-w-2xl'}
    showCloseButton={false}
    onOpenAutoFocus={(event) => {
      if (!modalFocusEl) return;
      event.preventDefault();
      queueMicrotask(() => modalFocusEl?.focus({ preventScroll: true }));
    }}
    onkeydown={(e) => onKeydown?.(e)}
  >
    {#if modal?.method === 'custom' && modal.interactive}
      <Dialog.Header class="sr-only">
        <Dialog.Title>{modal.title || 'Extension terminal'}</Dialog.Title>
        <Dialog.Description id="extension-terminal-instructions">
    ...
    {:else}
      ...
      <Dialog.Header class="border-b border-base-content/8 px-5 py-4">
        ...
        {#if modal?.method === 'confirm' && modal.message}
          ...
          >
        {:else}
          <Dialog.Description class="sr-only">Extension UI dialog</Dialog.Description>
        {/if}
      </Dialog.Header>
