<script lang="ts">
  ...
  function onKeydown(e: KeyboardEvent) {
    if (e.key === 'Enter') {
      e.preventDefault();
      if (highlight >= 0 && highlight < completions.length)
        acceptCompletion(completions[highlight]);
      else submit();
      ...
      if (completions.length > 0)
        highlight = highlight < completions.length - 1 ? highlight + 1 : 0;
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      if (completions.length > 0)
        highlight = highlight > 0 ? highlight - 1 : completions.length - 1;
...
<div class="space-y-2">
  <div
    class="flex items-center gap-2 rounded-[1.4rem] border border-base-content/14 bg-base-content/[0.035] px-3.5 sm:px-4 py-2.5 sm:py-3 shadow-inner shadow-black/10 transition-colors focus-within:border-base-content/55 focus-within:bg-base-content/[0.045]"
  >
    ...
    {#if value.trim()}
      <button
        ...
        tabindex={tabbable ? 0 : -1}>{submitLabel}</button
      >
    {:else if onCancel}
      <button
        onclick={onCancel}
        ...
        tabindex={tabbable ? 0 : -1}><X class="w-3.5 h-3.5" /></button
      >
    {/if}
  </div>

  <div
    class="rounded-2xl bg-base-content/[0.035] border border-base-content/6 px-2.5 sm:px-3 py-2.5 min-h-12"
  >
    {#if value.trim()}
      <button
        onclick={submit}
      ...
      >
        ...
        <span class="min-w-0 flex-1 truncate font-mono text-sm text-base-content/75"
          >{value.trim()}</span
        >
    ...
    {#if completions.length > 0}
      <div
        class="mt-2 flex max-h-[min(11rem,28dvh)] flex-col gap-0.5 overflow-y-auto overscroll-contain border-t border-base-content/8 pt-2"
        role="listbox"
        aria-label="Directory completions"
      >
        {#each completions as entry, i (entry)}
          <button
            ...
            class="flex items-center gap-2 rounded-lg px-2 py-2 sm:py-1.5 text-left text-xs transition-colors {i ===
            highlight
              ? 'text-base-content/90 bg-base-content/12'
              : 'text-base-content/58 hover:text-base-content/90 hover:bg-base-content/8'}"
            tabindex={tabbable ? 0 : -1}
          >
