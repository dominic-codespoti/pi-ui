...
>
    ...
    {#if modelTab === 'models'}
      {#if model?.reasoning}
        <div class="shrink-0 px-5 py-3.5 border-b border-base-content/8">
          <p
            class="text-[10px] text-base-content/35 uppercase tracking-[0.12em] mb-3 font-semibold"
          >
            thinking
          </p>
  ...
  {:else if tab === 'tools'}
          ...
          {#if builtinTools.length > 0}
            ...
            {#each builtinTools as tool (tool.name)}
              ...
              <button
                ...
                title={isActive ? `Disable ${tool.name}` : `Enable ${tool.name}`}
              >
                {#if isActive}<span
                    class="absolute left-0 top-1 bottom-1 w-0.5 rounded-r-full bg-primary glow-primary"
                  ></span>{/if}
                <span class="min-w-0 flex-1">
                  <span class="text-sm font-mono text-base-content/80 block truncate"
                    >{tool.name}</span
                  >
                  {#if tool.description}<span
                      class="text-xs text-base-content/40 leading-relaxed line-clamp-2"
                ...
                <span
                  ...
                  aria-hidden="true"
                  class="cursor-pointer shrink-0 w-4 h-4 rounded border flex items-center justify-center transition-colors {isActive
    ...
    <div
      class="shrink-0 border-t border-base-content/10 px-5 py-2 flex items-center justify-between"
    >
      <button
  ...
  {:else}
          ...
          <div class="px-5 py-6 space-y-3 animate-pulse">
            {#each [0, 1, 2, 3] as i (i)}
              <div class="flex items-center gap-3">
                ...
                <div class="flex-1 space-y-1.5">
                  <div
                    class="h-3 bg-base-content/8 rounded w-{['1/2', '2/5', '1/3', '3/5'][i]}"
                  ></div>
                  <div
                    class="h-2 bg-base-content/5 rounded w-{['3/4', '1/2', '2/3', '2/5'][i]}"
                  ></div>
                </div>
              </div>
            {/each}
        ...
        {:else}
          ...
          {#if filteredSkills.prompts.length > 0}
            ...
            {#if projectPrompts.length > 0}{@render sectionHeader(
                ...
                'bg-primary/70',
                'project prompts'
              )}{#each projectPrompts as prompt (prompt.name)}{@render promptItem(
                  prompt
                )}{/each}{/if}
    ...
    <div class="shrink-0 border-t border-base-content/10 px-5 py-3 space-y-2">
      ...
      {#if skillInstallFeedback}
        <p
          class="text-xs {skillInstallFeedback.success
            ? 'text-success'
            : 'text-error'} leading-snug"
        >
