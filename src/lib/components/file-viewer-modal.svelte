...
<Dialog.Root {open}>
  ...
  >
    ...
    <div class="flex items-center gap-3 px-4 py-2.5 border-b border-base-content/8">
      ...
      <Dialog.Title class="text-sm leading-normal font-medium truncate flex-1 min-w-0"
        >{path}</Dialog.Title
      >
      {#if line}
      ...
      {:else}
        <div class="file-content font-mono text-xs leading-[1.5] select-text">
          {#each visibleLines as contentLine, i (i)}
            {@const lineNum = i + 1}
            <div class="file-line" class:target-line={line && lineNum === line} data-line={lineNum}>
          ...
          {#if linesTruncated}
            <div class="px-4 py-2 text-[10px] text-base-content/40 italic">
              File exceeds {FILE_LINE_LIMIT} lines — only the first {FILE_LINE_LIMIT.toLocaleString()}
              are shown.
            </div>
          {/if}
        </div>
