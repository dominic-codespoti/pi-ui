      ...
      children: [
        {
          kind: 'text',
          label: '',
          content: 'Welcome to the extension lab! Try editing this JSON.',
        },
      ...
      examples: [
        {
          ...
          comp: {
            kind: 'text',
            label: '',
            content: 'This is a plain text block used for descriptions and labels.',
          },
        },
        {
            ...
            children: [
              {
                kind: 'text',
                label: '',
                content: 'Choose a model for this conversation:',
              },
            ...
            children: [
              {
                ...
                label: '',
                content:
                  'Are you sure you want to delete the project "my-app"? This action cannot be undone.',
              },
  ...
  {#if dev}
    ...
    <section class="mb-10 rounded-xl border border-base-content/10 bg-base-200/50 p-4">
      ...
      <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div>
          <label for="live-json" class="mb-1 block text-xs text-base-content/50"
            >ParsedComponent JSON</label
          >
          <textarea
            id="live-json"
        ...
        <div>
          ...
          {:else if !editorError}
            <div
              class="flex h-64 items-center justify-center rounded-lg border border-dashed border-base-content/12 text-xs text-base-content/30"
            >
    ...
    <section class="mb-10">
      ...
      {#each catalog as section (section.section)}
        <div class="mb-6">
          <h3 class="mb-3 text-xs font-semibold uppercase tracking-[0.15em] text-base-content/40">
            {section.section}
          </h3>
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {#each section.examples as ex (ex.name)}
              <div class="rounded-xl border border-base-content/8 bg-base-200/30 p-4">
                ...
                <details class="group">
                  <summary
                    class="cursor-pointer text-xs text-base-content/40 hover:text-base-content/60"
                  >
                    JSON
                  </summary>
                  <pre
                    class="mt-2 overflow-x-auto rounded bg-base-300/50 p-2 text-[10px] leading-relaxed text-base-content/50"><code
                      >{JSON.stringify(ex.comp, null, 2)}</code
                    ></pre>
                </details>
              </div>
            {/each}
    ...
    <section class="mb-10 rounded-xl border border-base-content/10 bg-base-200/50 p-4">
      ...
      {#if log.length === 0}
        <p class="text-xs text-base-content/30">
          Click interactive elements above to see events here.
        </p>
      {:else}
        <div class="max-h-48 space-y-0.5 overflow-y-auto font-mono text-xs">
          {#each log as entry (entry.time + entry.value)}
            <div class="rounded bg-base-300/30 px-2 py-1 text-base-content/60">
              <span class="text-base-content/40">{entry.time}</span>
              {entry.value}
            </div>
          {/each}
        </div>
