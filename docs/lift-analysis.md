...
4. **Unread/attention states in the session list.** OpenClaw's unread dot + "needs input" chip is the biggest UX gap vs. our list. Since the pivot to a single live session there is no in-memory unread/running state for non-live sessions, so this must be built on the disk-backed catalog: extend it with `lastReadAt` (localStorage or server setting) and derive `needsInput` from the session file plus the SW notification path (which knows when a turn ends hidden). _Effort: M._
